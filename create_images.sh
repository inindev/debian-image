...
setup_dtb() {
    ...
    # boards not yet carried by the debian kernel take their device trees
    # from the inindev kernel package: these are installed version-independently
    # to /boot/dtbs/local and are overlaid onto each kernel's dtb directory by
    # the dtb_cp hook, whichever kernel that image runs
    local dtb_src="$dl_dir/dtbs"
    extract_dtbs "$dl_dir/kernel/inindev.deb" 'rk3566-orangepi-cm4*' "$dtb_src"
    [ -d "$dtb_src" ] || return 0
...
extract_dtbs() {
    ...
    # each rootfs flavor calls this, but the dtbs come from one kernel package
    [ -d "$out_dir" ] && return 0

    phead "extracting device trees: $dtb_filter"

    # build_rootfs owns the EXIT trap that unmounts media, so clean up
    # explicitly rather than installing one here
    local tmpdir="$(mktemp -d)"

    ar x --output="$tmpdir" "$kern_deb" 'data.tar.xz'
    tar -C "$tmpdir" --strip-components=4 --wildcards -xavf "$tmpdir/data.tar.xz" \
        "./usr/lib/linux-image-*-arm64/rockchip/$dtb_filter"

    mkdir -p "$out_dir"
    mv "$tmpdir/rockchip/"* "$out_dir"
    echo "device trees extracted to $out_dir"
